...
const PropertyList: React.FC = () => {
    ...
    // );
    return (
        <section className="mx-auto w-full max-w-7xl space-y-4 rounded-2xl border border-slate-700 bg-slate-900 p-5 text-slate-100">
            <header className="space-y-1">
                <h2 className="text-2xl font-semibold">Lista de Propiedades</h2>
                <p className="text-sm text-slate-400">Vista general estructurada con jerarquía consistente.</p>
            </header>
                ...
                <table className="w-full text-left text-sm text-slate-200">
                    <thead className="bg-slate-800 text-xs uppercase tracking-wide text-slate-300">
                        <tr>
                            ...
                            <th scope="col" className="px-4 py-3">Propiedad</th>
                            <th scope="col" className="px-4 py-3">Ubicación</th>
                            <th scope="col" className="px-4 py-3">Destino</th>
                            <th scope="col" className="px-4 py-3">Estado</th>
                            <th scope="col" className="px-4 py-3"><span className="sr-only">Modificar</span></th>
                            <th scope="col" className="px-4 py-3"><span className="sr-only">Eliminar</span></th>
                        </tr>
                    </thead>
                    <tbody>
                        {properties.map((property) => (
                            <tr key={property.id} className="border-t border-slate-700 bg-slate-900/60">
                                <td className="px-4 py-3">{property.id}</td>
                                <td className="px-4 py-3">{property.classification}</td>
                                <td className="px-4 py-3">{property.province}</td>
                                <td className="px-4 py-3">{property.destinyUse}</td>
                                <td className="px-4 py-3">{property.status}</td>
                                <td className="px-4 py-3 text-right">
                                    <a href={`/property/${property.id}/modify-property`} className="font-medium text-blue-400 hover:text-blue-300 hover:underline">Modificar</a>
                                </td>
                                <td className="px-4 py-3 text-right">
                                    <a href="#" className="font-medium text-blue-400 hover:text-blue-300 hover:underline">Eliminar</a>
                                </td>
                            </tr>
                        ))}
                    </tbody>
